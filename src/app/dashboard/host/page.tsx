...
import { Gauge, Users, Zap, ArrowLeft, Search } from "lucide-react";
...
export default function HostDashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <Link
        href="/dashboard"
        className="mb-4 inline-flex items-center gap-1 text-xs font-medium text-navy-light/60 hover:text-navy"
      >
      ...
      <div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <select
          ...
          className="rounded-xl border border-surface-muted bg-surface-card px-4 py-2.5 text-sm text-navy outline-none focus:border-gold focus:ring-2 focus:ring-gold/30"
        >
          {SOLAR_PLANTS.map((p) => (
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-4">
        <Card>
          <CardContent className="p-5">
            ...
            <p className="font-display text-xl font-bold text-navy">{plant.capacityKw} kW</p>
            <p className="text-xs text-navy-light/60">Plant Capacity</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
            ...
            <p className="text-xs text-navy-light/60">Active Subscribers</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
            ...
            <p className="text-xs text-navy-light/60">Grid Output / Month</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
            ...
            <p className="font-display text-xl font-bold text-navy">{formatINR(totalCredits)}</p>
            <p className="text-xs text-navy-light/60">Disbursed to residents / month</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="mb-4 flex flex-col gap-3 sm:flex-row">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-xl border border-surface-muted bg-surface py-2 pl-9 pr-4 text-sm text-navy outline-none focus:border-gold focus:ring-2 focus:ring-gold/30"
              />
            </div>
            <select
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value as typeof statusFilter)}
              className="rounded-xl border border-surface-muted bg-surface px-3 py-2 text-sm text-navy outline-none focus:border-gold focus:ring-2 focus:ring-gold/30"
            >
          ...
          {visibleSubscribers.length === 0 && (
            <p className="py-6 text-center text-sm text-navy-light/50">
              No subscribers match this search/filter.
            </p>
          )}
          {visibleSubscribers.length > 25 && (
            <p className="mt-3 text-center font-mono text-xs text-navy-light/50">
